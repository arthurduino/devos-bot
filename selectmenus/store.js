module.exports = {
  async run({ client, interaction }) {
    ...
    if (!userDB) return interaction.error(`${interaction.user.toString()}, Votre profil n'est pas enregistré. Faites la commande \`/create-profile\` ou envoyez un message pour enregistrer un profil.`);
    ...
    if (item.item == 'ad_role') {
      ...
      interaction.success(`${interaction.user.toString()}, Vous avez correctement acheter ce produit. Je vous ai débité 10 credits.`);
    }
  }
};